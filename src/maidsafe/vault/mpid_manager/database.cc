...
//std::unique_ptr<MpidManager::Value> MpidManagerDataBase::Commit(const MpidManager::Key& key,
//    std::function<detail::DbAction(std::unique_ptr<MpidManager::Value>& value)> functor) {
//  assert(functor);
//  std::unique_ptr<MpidManager::Value> value;
//  try {
//    value.reset(new MpidManager::Value(Get(key)));
//  }
//  catch (const maidsafe_error& error) {
//    if (error.code() != make_error_code(VaultErrors::no_such_account)) {
//      LOG(kError) << "MpidManagerDataBase::Commit unknown db error "
...
//    assert(value);
//    LOG(kInfo) << "MpidManagerDataBase::Commit putting entry";
//    Put(key, std::move(*value));
//  } else {
//    LOG(kInfo) << "MpidManagerDataBase::Commit deleting entry";
//    if (!value)
...
//void MpidManagerDataBase::Put(const MpidManager::Key& key, const MpidManager::Value& value) {
//  if (!data_base_)
...
//MpidManager::Value MpidManagerDataBase::Get(const MpidManager::Key& key) {
//  if (!data_base_)
//    BOOST_THROW_EXCEPTION(MakeError(CommonErrors::db_not_presented));
//  MpidManager::Value value;
//  std::string query(
//      "SELECT Chunk_Size, Storage_Nodes FROM MpidManagerAccounts WHERE Chunk_Name=?");
//  sqlite::Statement statement{*data_base_, query};
...
//std::map<MpidManager::Key, MpidManager::Value> MpidManagerDataBase::GetRelatedAccounts(
//    const PmidName& pmid_name) {
//  if (!data_base_)
//    BOOST_THROW_EXCEPTION(MakeError(CommonErrors::db_not_presented));

//  std::map<MpidManager::Key, MpidManager::Value> result;
//  std::string query(
//      "SELECT * FROM MpidManagerAccounts WHERE Storage_Nodes LIKE ?");
//  sqlite::Statement statement{*data_base_, query};
...
//    MpidManager::Key key(ComposeKey(statement.ColumnText(0)));
//    MpidManagerValue value(ComposeValue(statement.ColumnText(1), statement.ColumnText(2)));
//    result[key] = value;
//  }
...
//  std::vector<MpidManager::Key> prune_vector;
//  MpidManager::TransferInfo transfer_info;

//  std::string query("SELECT * from MpidManagerAccounts");
//  sqlite::Statement statement{*data_base_, query};
//  while (statement.Step() == sqlite::StepResult::kSqliteRow) {
//    MpidManager::Key key(ComposeKey(statement.ColumnText(0)));
//    MpidManager::Value value(ComposeValue(statement.ColumnText(1), statement.ColumnText(2)));
...
//                   << HexSubstr(key.name.string()) << " to " << check_holder_result.new_holder;
//        std::vector<MpidManager::KvPair> kv_pair;
...
//void MpidManagerDataBase::HandleTransfer(const std::vector<MpidManager::KvPair>& contents) {
//  LOG(kVerbose) << "MpidManager AcoccountTransfer MpidManagerDataBase::HandleTransfer";
//  for (const auto& kv_pair : contents) {
//    try {
//      Get(kv_pair.first);
//    }
//    catch (const maidsafe_error& error) {
//      LOG(kInfo) << "MpidManager AcoccountTransfer MpidManagerDataBase::HandleTransfer "
//                 << error.what();
...
//MpidManager::Value MpidManagerDataBase::ComposeValue(const std::string& chunk_size,
